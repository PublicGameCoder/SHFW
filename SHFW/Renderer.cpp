...
void Renderer::_renderSpriteBatch(glm::mat4 modelMatrix, std::vector<Sprite*>& spritebatch, Camera* camera)
{
	...
	Shader* shader = _resman.getShader(spr->getVertexShader().c_str(), spr->getFragmentShader().c_str());
	// ask resourcemanager
	if (shader == NULL) {
		shader = _defaultShader; // fallback to defaultshader
	}
	std::string texturename = spr->getTextureName();
	int filter = spr->getFilter();
	int wrap = spr->getWrap();
	...
	Mesh* mesh = _resman.getSpriteMesh(spr->size.x, spr->size.y, spr->pivot.x, spr->pivot.y, spr->uvdim.x, spr->uvdim.y, spr->getCircleMesh(), spr->isWhich());
	...
	if (texture != NULL) {
		...
		for (int i = 0; i < s; i++) {
			...
			if (sprite->useCulling()) { // but maybe we don't
				int half_width = MINWINDOWWIDTH /2;
				int half_height = MINWINDOWHEIGHT /2;
...
{
	Shader* shader = _resman.getShader(sprite->getVertexShader().c_str(), sprite->getFragmentShader().c_str());
	if (shader == NULL) {
		shader = _defaultShader; // fallback to defaultshader
	}
	...
	if (dynamic) {
		if (sprite->getTexture() != NULL) {
			texture = new Texture();
			texture->createFromBuffer(sprite->getTexture()->pixels());
		}
	} else {
		texture = _resman.getTexture(sprite->getTextureName(), sprite->getFilter(), sprite->getWrap());
	}
	...
	if (sprite->getPalette() != NULL) {
		...
		glBindTexture(GL_TEXTURE_1D, sprite->getPalette()->getGLTexture());
