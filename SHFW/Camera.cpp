...
void Camera::perspective()
{
	...
	_projectionMatrix = glm::perspective(45.0f, (float)MINWINDOWWIDTH /(float)MINWINDOWHEIGHT, 0.1f, 10000.0f);
...
void Camera::orthogonal()
{
	...
	_offset = Point3(MINWINDOWWIDTH /2, MINWINDOWHEIGHT /2, 0);

	// Initial position : on +Z
	// Unintuitively, this is the centre of the screen, so add _offset
	position = Point3( 0, 0, 650 ) + _offset;

	_projectionMatrix = glm::ortho(0.0f, (float)MINWINDOWWIDTH, (float)MINWINDOWHEIGHT, 0.0f, 0.1f, 1000.0f);
