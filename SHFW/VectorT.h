...
public:
	...
	*/
	...
	// distance, angle, normalize, dot
	/**
	* @brief Get the distance of the Vector_t<T>
	*
	* Returns the distance of the Vector_t<T> (pythagoras)
	*
	* @return T
	* The distance of the Vector_t<T>
	*/
	...
	*
	* @return Vector_t<T>
	* Returns a Vector_t<T> with distance 1.
	*/
	...
	*/
	...
	/**
	* @brief Limit the distance of this Vector2 to the amount.
	*
	* @param amount
	...
	*/
	...
	void applyForce(VectorX_t<T> other);

	void applyForceByMass(VectorX_t<T> other, T mass);

	const VectorX_t<T> copy();
	...
	*
	* @param other
	* The VectorX_t<T> to compare with (compares the distance of the VectorX_t<T>)
	*
	* @return
...
template <class T>
const T VectorX_t<T>::distance() const
{
...
{
	if (this->distSquared() > amount*amount) {
		this->normalize();
		*this *= amount;
	}
}
...
template <class T>
void VectorX_t<T>::applyForce(VectorX_t<T> other)
{
	VectorX_t<T> f = other.copy();
	this += f;
}

template <class T>
void VectorX_t<T>::applyForceByMass(VectorX_t<T> other, T mass)
{
	VectorX_t<T> f = other.copy();
	f /= mass;
	this += f;
...
template <class T>
const VectorX_t<T> VectorX_t<T>::copy()
{
	T xx = this->x;
	T yy = this->y;
	T zz = this->z;

	return new VectorX_t<T>(xx,yy,zz);
}
